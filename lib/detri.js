...
(function () { 'use strict';
  ...
  function Detri(ephemeris, timestamp, latitude, longitude) {
    ...
    if (midnight > now) { // FIXME: Bug during a few minutes before midnight
      midnight = Find.midnight(now - 86400000, lat, lon);
    }
    ...
    var day = (now - midnight) / duration;
    var c = (day * 100);
    var b = (day * 100000) % 1000;
  ...
  Detri.prototype.toDate = function() {
    ...
    var m = this.month;
    var d = this.day;
    return sprintf('%03d:%02d:%02d', y, m, d);
  }

  Detri.prototype.toTime = function() {
    var c = this.centiday;
    var b = this.beat;
    return sprintf('%02d:%03d', c, b);
  }
